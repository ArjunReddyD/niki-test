...
public class SignupPage extends keyword{
	
	readFromProperties prop = new readFromProperties();
	
	public SignupPage(AndroidDriver andriver) {
	...
	public void enterFormDetails(String name, String email, String otpValue) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("otp value in formdetails:"+otpValue);
		type(By.id(prop.getAppProperty("niki_signup_name_id")),name);
		click(By.id(prop.getAppProperty("niki_signup_email_id")));
		click(By.name(email));
		type(By.id(prop.getAppProperty("niki_signup_otp_id")),otpValue);
		click(By.name(prop.getAppProperty("niki_signup_btn_name")));
	}
